#' Extract summary statistics from a VCF file (multi-sample support)
#'
...
#'
#' @param vcf.file Path to a bgzipped VCF file (.vcf.gz). Must be indexed with tabix (.tbi).
#' @param fields Character vector of FORMAT fields to extract (e.g., \code{c("ES", "SE", "LP")}).
#' @param regions Optional character vector of genomic regions to extract, in the format \code{"chr:start-end"}.
#'
#' @return A \code{data.frame} with standard VCF columns and one column per sample × field (e.g., \code{S1_ES}, \code{S2_ES}).
#'
#' @examples
...
#'
#' @export
read.summary.stats <- function(vcf.file, fields = c("ES", "SE", "LP"), regions = NULL) {
  if (!file.exists(vcf.file)) stop("Fichier VCF non trouvé: ", vcf.file)
  if (!grepl("\\.vcf\\.gz$", vcf.file)) stop("Le fichier doit être un .vcf.gz (compressé bgzip)")
  if (!is.character(fields) || length(fields) == 0) stop("Argument 'fields' invalide.")
  if (!is.null(regions) && !is.character(regions)) stop("regions doit être NULL ou un vecteur de chaînes.")
  
  readSummaryStats(vcf.file, fields, regions)
}
